...
function mapStateToProps({ userProperties }, ownProps) {
  const currentBuilding = userProperties.buildings.filter(
    building => building._id === ownProps.match.params.id
  );
  return {
    userProperties,
    initialValues: {
      ...
      certifications: currentBuilding[0]
        ? currentBuilding[0].certifications
        : ''
    }
  };
}
